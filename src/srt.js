...
Srt.prototype = {
        ...
        for (var i = 0; i < lines.length; i++) {
            ...
            if (origin.length >= 3) {
              ...
              //  var endDate = this.stringToDate(endText);
                var startsec = this.stringToSecs(startText);
                var endsec = this.stringToSecs(endText);
                // subtitle 
                var subtitle = '';
    ...
    stringToSecs:function(string){
        var firstColonIndex = string.indexOf(':');
        var secondColonIndex = this.nthChar(string, ':', 2);
        var commaIndex = string.indexOf(',');
        var hour = parseInt(string.substring(0, firstColonIndex));
        var minute = parseInt(string.substring(firstColonIndex + 1, secondColonIndex));
        var second = parseInt(string.substring(secondColonIndex + 1, commaIndex));
        var msecond = parseInt(string.substring(commaIndex + 1));
        var totalsec = hour * 3600 + minute * 60 + second;
        totalsec += msecond / 1000.0;
        return totalsec
    },
